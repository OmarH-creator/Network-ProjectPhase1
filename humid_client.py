...
class HumidityClient:
    def __init__(self, device_id, host, port, interval, seed=None, heartbeat_interval=10.0, enable_heartbeat=False, period_heartbeat=3.0, enable_batching=False, batching_interval=10.0):
        ...
        if enable_batching:
            print(f"[HUMID CLIENT {device_id}] Batching enabled: {batching_interval}s batch interval, max {self.max_readings_per_packet} readings/packet")
    ...
    def generate_humidity_reading(self):
        """Generate a single humidity reading"""
        humid_value = random.uniform(30.0, 90.0)
        return SensorReading(SENSOR_HUM, humid_value)

    def send_humidity_data(self):
        """Send single humidity reading (normal mode)"""
        reading = self.generate_humidity_reading()
        readings = [reading]
        ...
        self.sock.sendto(encode_packet(packet), (self.host, self.port))
        print(f"[HUMID CLIENT {self.device_id}] DATA seq={self.seq}, humidity={reading.value:.2f}%")
        self.seq += 1
    ...
    def add_reading_to_batch(self):
        ...
        print(f"[HUMID CLIENT {self.device_id}] Added to batch: humidity={reading.value:.2f}% (batch size: {len(self.batch_readings)})")
        
        # Check if batch is full
        if len(self.batch_readings) >= self.max_readings_per_packet:
            print(f"[HUMID CLIENT {self.device_id}] Batch full ({self.max_readings_per_packet} readings), sending early")
            self.send_batch()
    ...
    def send_batch(self):
        ...
        # Log batch details
        humid_values = [r.value for r in self.batch_readings]
        avg_humid = sum(humid_values) / len(humid_values)
        min_humid = min(humid_values)
        max_humid = max(humid_values)
        
        print(f"[HUMID CLIENT {self.device_id}] BATCH seq={self.seq}, {len(self.batch_readings)} readings, "
              f"humidity avg={avg_humid:.2f}% (min={min_humid:.2f}, max={max_humid:.2f})")
    ...
    def run(self, duration):
        ...
        if self.enable_batching:
            # BATCHING MODE
            print(f"[HUMID CLIENT {self.device_id}] Running in BATCHING mode")
            ...
            try:
                ...
                if self.batch_readings:
                    print(f"[HUMID CLIENT {self.device_id}] Sending final batch with {len(self.batch_readings)} readings")
                    self.send_batch()
            ...
            print(f"[HUMID CLIENT {self.device_id}] Running in NORMAL mode")
